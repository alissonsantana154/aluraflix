...
import dominiosProibidos from "../../database/db.json"
...
function ModalEditar({ aberto, fechado, cardId }) {
      ...
      const requisicaoDadosCard = async () => {
        try {
          const resposta = await fetch(
            `https://668c6b2b0b61b8d23b0d4f6f.mockapi.io/cards/${cardId}`
          )
  ...
  const aoSalvar = async () => {
    ...
    const dominioProibidoEncontrado = dominiosProibidos.some((proibido) =>
      video.includes(proibido)
    )
    ...
    <>
      ...
      <DialogEstilizado aberto={aberto}>
        ...
        <Form>
          ...
          <BoxBotoes>
            <Botao type="button" onClick={aoSalvar}>
              SALVAR
            </Botao>
            <Botao type="button" onClick={limparInputs}>
              LIMPAR
            </Botao>
          </BoxBotoes>
        </Form>
